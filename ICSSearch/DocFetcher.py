...
def CreateTrie():
    ...
    print ("                          |\r", end = "")
    if not len(os.listdir(trieFolder)) == 26:
        indexsrc = src + "Index/"
        for file in os.listdir(indexsrc):
            jsonData = json.load(open(indexsrc + file, "r", encoding="utf8"))
            char = file[-6]
            ...
            f = io.open(trieFolder + char + ".json", "w", encoding = "utf8")
            f.write(json.dumps(Trie.IndexTrie[char], ensure_ascii = False))
            f.close()
            print ("=", sep="", end="")
        print ("\nLoad Complete in: ", time.clock() - start)
        VerifyTrie()
    else:
        print("Load Complete (Pregenerated)")
...
def GetResult(query, respDict):
    ...
    if result != []:
        ...
        respDict[current_thread().name] = finalresult
        return(finalresult)
...
def GetASResult(query, respDict):
    start = time.clock()
    tokens = nltk.word_tokenize(query)
    result = [""]
    for token in tokens:
    ...
    timetaken = time.clock() - start
    print("Fetched in ", timetaken, " secs")
    respDict[current_thread().name] = result[:5]
    return result[:5]